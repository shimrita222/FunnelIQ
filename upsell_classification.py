# %% [markdown]
# # Package 3 – Upsell Classification
#
...
# Business-rule thresholds (dataset medians): flag a customer for outreach if
# their lifetime is above-median and their acquisition cost is below-median.
BUSINESS_RULE_LTV_THRESHOLD = 21
...
#
# `cumulative_profit` and `referred` are excluded from the feature set, for the
# same reason as in Package 2: both are outcomes that only exist once the full
# customer relationship has played out, so they wouldn't be available at the
# moment an upsell prediction is actually needed. `ltv_months` and the rest of
# the funnel columns are kept as features per this package's brief.
...
#
# A simple, explainable baseline: flag a customer for upsell outreach if their
# lifetime is above the dataset median and their acquisition cost is below the
# dataset median — no model required.


# %%
def apply_business_rule(data: pd.DataFrame) -> pd.Series:
    """Flag customers for outreach: above-median LTV and below-median CAC."""
    return (
        (data["ltv_months"] > BUSINESS_RULE_LTV_THRESHOLD)
